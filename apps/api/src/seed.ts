...
async function main() {
  ...
  // Locations
  const warehouse = await prisma.location.upsert({ where: { id: 'loc-warehouse-001' }, update: {}, create: { id: 'loc-warehouse-001', name: 'Main Warehouse', address: '100 Industrial Blvd, Surrey BC', isDefault: true } }).catch(() => prisma.location.findFirst({ where: { name: 'Main Warehouse' } })) as any;
  const factory = await prisma.location.upsert({ where: { id: 'loc-factory-001' }, update: {}, create: { id: 'loc-factory-001', name: 'Factory Floor', address: '100 Industrial Blvd — Building B' } }).catch(() => prisma.location.findFirst({ where: { name: 'Factory Floor' } })) as any;
  ...
  // Inventory (via direct upsert for seed)
  ...
  await prisma.batch.createMany({ data: [
    { variantId: bikeRed.id, batchNumber: 'BATCH-2026-001', expiryDate: new Date('2028-12-31'), notes: 'Q1 production run' },
    { variantId: bikeBlue.id, batchNumber: 'BATCH-2026-002', notes: 'Q1 production run' },
  ]});
  ...
  // Inventory movements (opening stock records)
