...
export default function PODetailPage() {
  ...
  const receive = useMutation({
    mutationFn: () => api.post(`/purchase-orders/${id}/receive`, { locationId: receiveLocationId, rows: (po?.rows || []).map((r: any) => ({ rowId: r.id, receivedQty: Number(receivedQty) || r.qty })) }),
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center gap-3">
        <Link href="/dashboard/purchase-orders" className="icon-btn"><ArrowLeft size={16} /></Link>
      ...
      <Modal open={receiveOpen} onClose={() => setReceiveOpen(false)} title="Receive Stock">
        <div className="space-y-3">
          ...
          <p className="text-xs text-gray-500">All outstanding quantities will be received into this location.</p>
        </div>
        <div className="flex justify-end gap-2 mt-4">
