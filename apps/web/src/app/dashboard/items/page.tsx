...
export default function ProductsPage() {
  ...
  const columns: Column[] = [
    ...
    { key: "skus", header: "SKU(s)", render: (r) => r.variants?.map((v: any) => v.sku).join(", ") || "—" },
    { key: "status", header: "Status", isStatus: true, render: (r) => <StatusCell status={r.status || (r.isActive !== false ? "active" : "inactive")} />, filterable: false },
    ...
    <>
      ...
      <div className="flex items-center justify-between px-4 py-2.5 bg-white border-b border-gray-200">
        ...
        <button className="btn-primary text-[13px] py-1.5 px-3 rounded-md inline-flex items-center gap-1.5 font-medium" onClick={openNew}>
          <Plus size={14} strokeWidth={2.5} />Product
        </button>
      </div>
      <div className="px-4 py-3">
        <DataTable columns={columns} data={data || []} isLoading={isLoading} emptyMessage="No products found" showRank countLabel="products" />
      </div>
