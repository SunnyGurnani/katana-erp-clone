...
import { Play, Square, Check, Clock, User, LogOut } from "lucide-react";
...
export default function ShopFloorApp() {
  ...
  if (!operatorId) {
    return (
      <div className="flex items-center justify-center h-screen bg-gray-50 page-transition">
  ...
  // Filter tasks: only show open/in_progress, sorted by priority (if we had it), then date
  const myTasks = (tasks || [])
                      ...
                      <div className="flex-1 p-5 sm:p-6 flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6">
                        <div className="flex-1">
                          ...
                          <div className="flex items-center gap-4 text-sm text-gray-500">
                            ...
                            <span className="flex items-center gap-1">
                              <Clock size={14} /> Est: {t.estimatedDuration || "—"} mins
                            </span>
                          </div>
                        </div>
                          ...
                          {isWIP ? (
                            <>
                              <button 
                                ...
                                className="flex flex-col items-center justify-center w-20 h-20 bg-amber-50 text-amber-600 rounded-xl hover:bg-amber-100 active:bg-amber-200 transition-colors border border-amber-200"
                              >
                              ...
                              <button 
                                ...
                                disabled={updateTask.isPending}
                                className="flex flex-col items-center justify-center w-20 h-20 bg-green-500 text-white rounded-xl hover:bg-green-600 active:bg-green-700 transition-colors shadow-sm"
                              >
                            ...
                            <button 
                              ...
                              disabled={updateTask.isPending}
                              className="flex items-center justify-center gap-2 h-20 px-8 bg-brand-600 text-white rounded-xl hover:bg-brand-700 active:bg-brand-800 transition-colors shadow-sm text-lg font-bold"
                            >
...
// Simple local component for the Wrench icon
